...
def set_default_parameters(name):
    ...
    params.setdefault('recurse_cooldown_N',3)
...
class phiPrec:
    ...
    def _is_anchor(self, x) -> bool:
        """
        Works for both numpy arrays and TorchVect. For TorchVect, checks
        per-parameter torch.allclose with (rtol=0, atol=self.x_tol).
        """
        if self._x_coarse is None:
            return False

        # TorchVect case
        if hasattr(x, "td") and hasattr(self._x_coarse, "td"):
            # quick shape sanity
            if set(x.td.keys()) != set(self._x_coarse.td.keys()):
                return False
            for k in x.td.keys():
                a = x.td[k]; b = self._x_coarse.td[k]
                if a.shape != b.shape:
                    return False
                if not torch.allclose(a, b, rtol=0.0, atol=float(self.x_tol)):
                    return False
            return True

        # Fallback: numpy/scalar-like
        return np.allclose(as_numpy(x), as_numpy(self._x_coarse),atol=float(self.x_tol), rtol=0.0)
    ...
    def prox(self, u:np.ndarray,t:float) -> np.ndarray:
        ...
        #        px = self.R @ self.problems[self.l-1].obj_nonsmooth.prox(self.R.T @ u ,t)
            ...
            coarse_pv = self.problems[self.l].pvector
            d_comp = coarse_pv.norm(xcomp - u)
            d_anc  = coarse_pv.norm(xanc  - u)

            Fcomp = 0.5/float(t) * (d_comp ** 2) + prev.value(self.R.T @ xcomp)
            Fanc  = 0.5/float(t) * (d_anc  ** 2) + prev.value(self.x_fine)
...
def trustregion_step(l, x, val, grad, problems, params, cnt, i=0):
    ...
    if l < L - 1:
        ...
        diff     = pgrad_c - rhs  
        prox_err = problems[l+1].pvector.norm(diff)
        tol_abs = params.get('prox_equiv_abs_tol', 1e-10)
        tol_rel = params.get('prox_equiv_rel_tol', 0.0)   
        if tol_rel > 0.0:
            ref = max(1e-16, problems[l+1].pvector.norm(pgrad_c) + problems[l+1].pvector.norm(rhs))
            tol = max(tol_abs, tol_rel * ref)
        else:
            tol = tol_abs
        prox_ok = (prox_err <= tol)
    ...
    if not prox_ok:
        print(f"[PROX-EQ l={l}] ||pgrad_c - rhs||={prox_err:.3e} tol={tol:.1e}")
    
    do_recurse = (l < L - 1) and (i > 0) and  drop_rel and drop_abs
...
def trustregion(l, x0, Deltai, problems, params): #inpute Deltai
    ...
    for i in range(1, params['maxit'] + 1):
        ...
        deff = params.get('delta_effective', params['delta'])
        on_boundary = (snorm >= 0.9 * max(1e-16, deff))
        ...
        if aRed < params['eta1'] * pRed:
            ...
            if aRed > params['eta2'] * pRed :#and on_boundary:
...
import torch, numpy as np, math, collections
...
# --- helpers: euclidean dot and type guards ---
...
def driver(savestats, name):
    ...
    #meshlist        = [NN_dim_fine, NN_dim_coarse, NN_dim_coarse_2]
    #meshlist = [np.array(d, dtype=int) for d in NN_dims]
    #L = len(meshlist)
    #n_per_level = n[0]
    #if isinstance(n_per_level, (int, np.integer)):
    #    nlist = [int(n_per_level)] * L
    #else:
    #    assert len(n_per_level) == L, "n_per_level must have one entry per level"
    #    nlist = list(map(int, n_per_level))
    ...
    def _build_R_for_level(next_sizes, cur_sizes, x_fine):
        ...
        return _attach_global_shape(R_try,x_fine)
    x_seed_next = None
    #nnset0 = NNSetup(meshlist[0], nlist[0], alpha, beta, n_samples=1)
    #var_warm = nnset0.returnVars(False); var_warm['lambda_reg'] = lambda_warm
    #x0 = adam_warmstart(nnset0, var_warm, steps=2000, lr=5e-3, batch=4096,
    #                    beta_l1=0.0, beta_l2=1e-6, plateau=200, lr_decay=0.5,
    #                    eval_every=50, eval_chunk=8192, use_ema=True, ema_decay=0.995)

    # ---- Push weights down to every level and build per-edge R ----
    #x_levels = [None]*L
    #Rs       = [None]*L
    #x_levels[0] = x0

    #for i in range(L-1):
    #    # R_i maps level i -> i+1, built using the *current* x_levels[i]
    #    R_i = _build_R_for_level(next_sizes=meshlist[i+1], cur_sizes=meshlist[i], x_fine=x_levels[i])
    #    # sanity
    #    for name, W in R_i.td.items():
    #        out_shape = R_i.shapes_map[name]
    #        in_size   = x_levels[i].td[name].numel()
    #        out_size  = int(torch.tensor(out_shape).prod().item()) if len(out_shape)>0 else 1
    #        assert W.shape == (out_size, in_size), f"{i}: {name} R shape {W.shape} != ({out_size},{in_size})"
    #    Rs[i] = R_i
    #    x_levels[i+1] = R_i @ x_levels[i]

    ## Coarsest R is identity on its own space
    #Rs[-1] = _attach_global_shape(Reye(x_levels[-1]), x_levels[-1])
    ...
    for i in range(len(meshlist)):
        ...
        if i == 0:
            ...
            x     = adam_warmstart(nnset, var_warm,steps=1000, lr=2e-3, batch=4096,beta_l1=0.0,beta_l2=1e-6, plateau=200, lr_decay=0.5,eval_every=50, eval_chunk=8192,target_L2_rel=None,target_res_RMSE=None,use_ema=True, ema_decay=0.995)
            x0 = x
    #        print("blub")
        else:
            x = x_seed_next
     ...
     #       print("second blub")
        ...
        var['beta'] = beta
    #for i in range(L):
    #    nnset_i = NNSetup(meshlist[i], nlist[i], alpha, beta, n_samples=1)
    #    var_i   = nnset_i.returnVars(False)
    #    var_i['beta'] = beta
    #    R_i = Rs[i]
