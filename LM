...
def LMTR(input_data, model, pde, real_solution, lambdak=0.1, regularization=False):
    ...
    with torch.no_grad():
        # Set weights and biases for the first hidden layer
        Model.hidden_layers[0].weight = nn.Parameter(torch.tensor(w10, dtype=torch.float32))
        Model.hidden_layers[0].bias = nn.Parameter(torch.tensor(b10, dtype=torch.float32))

        # Set weights and biases for the output layer
        Model.output_layer.weight = nn.Parameter(torch.tensor(w20, dtype=torch.float32))
        Model.output_layer.bias = nn.Parameter(torch.tensor(b20, dtype=torch.float32))
    
    model_NN = model_nn(input_data,model = Model)
    Loss = loss_solve_pde(model_NN, np.linspace(0,1,40),regularization=True,lambdak=0.1)
    ###Optional parameters
    eta1 = 0.1
    eta2 = 0.75
    gamma1 = 0.85
    ...
    loss = compute_loss(input_data_tensor, model_tt, PDE((0,1), real_solution_1d), real_solution_1d, lambdak=0.1, regularization=True)
    ...
    # Access gradients
    gradients = torch.cat([param.grad.view(-1) for param in model_tt.parameters()])
    w10_1d = w10.squeeze()
    w20_1d = w20.squeeze()
    allparams = torch.cat([w10_1d,b10,w20_1d,b20])
    while k<=max_iter and np.linalg.norm(gradients)<=epsilon:
        def taylor_loss(s,input_data, model, pde, real_solution, lambdak=0.1, regularization=False):
            ...
            J_k = Jk(input_data_tensor, model, PDE((0,1), real_solution_1d), real_solution_1d)
        ...
        def line_A(input_data, model, pde, real_solution, lambdak=0.1, regularization=False):
            F_k = Fk(input_data_tensor, model, PDE((0,1), real_solution_1d), real_solution_1d)
            J_k = Jk(input_data_tensor, model, PDE((0,1), real_solution_1d), real_solution_1d)
            B_k = J_k.T@J_k
            return B_k
        def line_b(input_data, model, pde, real_solution, lambdak=0.1, regularization=False):
            F_k = Fk(input_data_tensor, model, PDE((0,1), real_solution_1d), real_solution_1d)
            J_k = Jk(input_data_tensor, model, PDE((0,1), real_solution_1d), real_solution_1d)
            return -1*J_k.T @F_k
        A_sub = line_A(input_data, model, pde, real_solution, lambdak=0.1, regularization=False)
        b_sub = line_b(input_data, model, pde, real_solution, lambdak=0.1, regularization=False)
        s = torch.zeros_like(b_sub)
        s = cg(A_sub,b_sub,s = s)
        pred = taylor_loss(torch.zeros_like(b_sub), input_data, model, pde, real_solution)-taylor_loss(s, input_data, model, pde, real_solution)
        fk = compute_loss(input_data, Model, pde, real_solution, lambdak=0.1, regularization=True)
        ...
        w20_new = new_params[2*r_nodes_per_layer:3*r_nodes_per_layer].vew((1,r_nodes_per_layer))
        b20_new = new_params[-1]
        with torch.no_grad():
            # Set weights and biases for the first hidden layer
            Model.hidden_layers[0].weight = nn.Parameter(torch.tensor(w10_new, dtype=torch.float32))
            Model.hidden_layers[0].bias = nn.Parameter(torch.tensor(b10_new, dtype=torch.float32))

            # Set weights and biases for the output layer
            Model.output_layer.weight = nn.Parameter(torch.tensor(w20_new, dtype=torch.float32))
            Model.output_layer.bias = nn.Parameter(torch.tensor(b20_new, dtype=torch.float32))
        
        model_NN = model_nn(input_data,model = Model)
        fks = compute_loss(input_data, Model, pde, real_solution, lambdak=0.1, regularization=True)
        ared = fk-fks
        pho = ared/pred
        if pho >= eta1:
            w10 = w10_new
            ...
            b20 = b20_new
            if pho>=eta2:
                lambdak = np.max(lambda_min,gamma2*lambdak)
            else:
                lambdak = np.max(lambda_min,gamma1*lambdak)
        else:
            w10 = w10
            b10 = b10
            w20 = w20
