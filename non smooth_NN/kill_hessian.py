...
#This model is a fully connected neural network with 2 hidden layers and each hidden layer has 400 nodes
#Loss function
...
def loss_grad(model, n_samples=10, meshsize = 31, reg_param=0.01):
    loss = loss_pde(model,n_samples,meshsize,reg_param)
    return torch.autograd.grad(loss,model.parameters(),create_graph=True)
    
#loss_grad_try = loss_grad(model)
#for i, grad in enumerate(loss_grad_try):
...
def loss_hessian(model,n_samples=10,meshsize = 31,reg_param = 0.01):
    ...
    return hessian
model_small = FullyConnectedNN(input_dim, 20, output_dim)

print(loss_hessian(model_small,n_samples=1).shape)

# Training Loop
def train_model(model,optimizer,n_samples=10,meshsize=31,reg_param=0.001):
    model.train()
...
def plot_comparison(model, n_samples=1, meshsize=31):
    ...
    predicted_solution = model(inputs).detach().numpy()
    
    # Convert solutions into 2D grids
    
    real_solution_grid = real_solution.reshape(meshsize + 1, meshsize + 1).numpy()
